...
interface ITerminalDimensions {
  ...
  /**
   * Amount of pixels required to make a full scrollbar.
   */
  extraSpace: number;
}
...
export class FitAddon implements ITerminalAddon {
  ...
  public fit(): void {
    ...
    if (this._terminal.rows !== dims.rows || this._terminal.cols !== dims.cols) {
      ...
      this._terminal.element!.style.paddingTop = `${dims.extraSpace}px`;
    }
  }
  ...
  public proposeDimensions(): ITerminalDimensions | undefined {
    ...
    const extraSpace = Math.max(0, parentElementHeight - elementPaddingVer - rows * core._renderService.dimensions.actualCellHeight);
    return {cols, rows, extraSpace};
  }
}
